...
export default class CustomerServiceTickets extends Component {
  ...
  findTickets(filters) {
    ...
    const { assigneeId, isOpen, status, categoryId, authorId,
      tagKey, tagValue, isOnlyUnlike, searchString, page = '0', size = '10', timeRange, showWatches } = filters
    ...
    return Promise.resolve()
      .then(() => {
        ...
        if (showWatches === 'true') {
          console.log('show watches ssss')
          result.push(new AV.Query('Watch')
      ...
      .then(([searchMatchedTicketIds, searchMatchedReplaies, watchs]) => {

        let ticketIds = []
        if (searchMatchedTicketIds.length + searchMatchedReplaies.length > 0) {
          ticketIds = _.union(searchMatchedTicketIds, searchMatchedReplaies.map(r => r.get('ticket').id))
        }
        if (showWatches === 'true') {
          const watchTicketIds = watchs.map(item => item.get('ticket').id)
          if (ticketIds.length > 0) {
            // 得到和搜索结果相交的 ids
            const unionIds = watchTicketIds.filter(id => ticketIds.includes(id))
            // 去重复
            ticketIds = Array.from(new Set(unionIds))
          } else {
            // 直接使用关注列表
            ticketIds = Array.from(new Set(watchTicketIds))
          }
  ...
  handleShowWatchesChange(e) {
    this.updateFilter({ showWatches: e.target.checked })
  }
  ...
  render() {
    ...
    const ticketAdminFilters = (
      <div className="formView">
        ...
        <div>
          <ButtonGroup>
            <Checkbox checked={filters.showWatches === 'true'} onChange={this.handleShowWatchesChange.bind(this)}>查看关注</Checkbox>
          </ButtonGroup>
