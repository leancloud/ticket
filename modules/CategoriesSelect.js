...
export default function CategoriesSelect({categoriesTree, selected, onChange, hiddenDisable = true}) {
  const options = _.compact(depthFirstSearchMap(categoriesTree, c => {
    if (hiddenDisable && c.get('deletedAt')) {
      return
    }
    return <option key={c.id} value={c.id} disabled={selected && (selected.id || selected.objectId) == c.id}>{getNodeIndentString(c) + getCategoryName(c)}</option>
  }))
  return (
    <FormControl
