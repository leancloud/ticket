...
import { addDays, differenceInSeconds, getDay, isAfter, isBefore, isSameDay, set } from "date-fns";
...
import { getTicketStatIds, getTicketDataById, parseTicketData, mergeData, MERGE_SEPARATOR } from "./utils";
...
const getReplyTimeConfig = async () => {
  ...
  config.workTime = differenceInSeconds(set(tmpDate,
    config.to
  ), set(tmpDate,
    config.from
  ))
...
export async function getTicketStatByDateRange(from: Date, to: Date) {
  ...
  await Promise.all(ids.map((id) => run(() => getTicketDataById(id, to).then(value => {
    if (!value) {
      return
    }
  ...
  const ticketStats: CreateData<TicketStats>[] = [
    ...
    ...Object.entries(results.category).map(([categoryId, statData]) => ({
      ...statData,
      categoryId,
      replyDetails: results.details.filter(v => v.categoryId === categoryId).map((value) => _.omit(value, 'categoryId', 'authorId'))
    })),
    ...Object.entries(results.customerService).map(([customerServiceId, statData]) => ({
      ...statData,
      customerServiceId,
      replyDetails: results.details.filter(v => v.authorId === customerServiceId).map((value) => _.omit(value, 'categoryId', 'authorId'))
    })),
    ...Object.entries(results.customerServiceCategory).map(([customerServiceCategory, statData]) => {
      const [customerServiceId, categoryId] = customerServiceCategory.split(MERGE_SEPARATOR)
      return {
        ...statData,
        categoryId,
        customerServiceId,
        replyDetails: results.details.filter(v => v.authorId === customerServiceId && v.categoryId === categoryId).map((value) => _.omit(value, 'categoryId', 'authorId'))
      }
    })
  ]
  return ticketStats;
