...
/** 获取需要统计的工单 id */
export const getTicketStatIds = async (from: Date, to: Date) => Promise.all([
...
const _getTicketTimeLine = (id: string, endDate: Date) => {
  ...
  const opsLogsPromise = OpsLog.queryBuilder()
    ...
    .orderBy('createdAt')
    .find(AUTH_OPTIONS);
  return Promise.all([repliesPromise, opsLogsPromise]).then(([replies, opsLogs]) => _.orderBy([...replies, ...opsLogs], 'createdAt'))
}
...
export const parseTicketData = (data: TicketData, start: Date, getRelyTime: (replyDate: Date, askDate: Date) => number) => {
  const {
    ticket,
    assigneeLogs,
    lifeLogs,
  ...
  const replyDetails = afterReplies.filter(v => !isBefore(v.createdAt, start)).map((v, index) => {
    const { cursor, ...rest } = v;
    const replyTime = cursor ? getRelyTime(v.createdAt, cursor.lastStaffReplyDate) : undefined;
    const naturalReplyTime = cursor ? differenceInSeconds(v.createdAt, cursor.lastStaffReplyDate) : undefined;
    let authorReplyTime = replyTime
    if (cursor && cursor.lastAssigneeDate && cursor.lastAssigneeId === v.authorId && isAfter(cursor.lastAssigneeDate, cursor.lastStaffReplyDate)) {
...
const _customizer = (objValue?: number, srcValue = 0) => objValue === undefined ? srcValue : objValue + srcValue
...
export const mergeData = (target: StatResult, source?: RawData) => {
  ...
  return {
    ...
    category: {
      ...target.category,
      [source.categoryId]: _.mergeWith({ ...target.category[source.categoryId] }, source.ticket, _customizer)
    },
